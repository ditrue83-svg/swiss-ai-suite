...
| Finanze | `/finanze` | sì | sì | sì | sì | parziale | sì | — | il codice QR **binario** non viene decodificato; le aliquote storiche non ci sono; su 4 voci reali 2 sono `completed` e 2 `failed` con `NOT_FINANCIAL`, che è una classificazione corretta. ⚠️ **Fatture emesse (0053/0054), misurato il 2026-09-02**: implementato **sì**, deployato **no** (le migrazioni non risultano applicate alla produzione e `generate-finance-invoice` non è distribuita al momento della scrittura), testato **parziale** (`test:finance-invoices-unit` **117/117** offline il 2026-09-02; la sezione 15 di `test:finance` è scritta e mai eseguita), servizio reale **no**, clienti esterni **no** — le sei colonne di questa riga descrivono il lato fornitori, in produzione dalla 0021 |
