...
| Clienti | `/clienti` | sì | — | sì | sì | sì | sì | Zefix (facoltativo) | l'abbinamento automatico non collega mai da solo: propone. ✅ **Import CSV (2026-08-28)**: wizard in tre passi su `/clienti/importa`, duplicati mostrati e mai fusi. Verificato: `test:crm-unit` 218/218, `test:crm` 94/94 eseguito il 2026-08-28 (sezione 14 compresa), prova browser su tenant usa-e-getta poi rimosso senza residui — flusso completo in italiano, file Windows-1252 con banner dichiarato, import da 200 righe con progresso, 375px senza scroll orizzontale, de/fr a campione (U+202F verificato nel DOM). ✅ **Campi personalizzati (0047, applicata il 2026-08-29)**: definizioni decise dall'azienda (nome, tipo, opzioni, obbligatorietà, ordine) e valori su organizzazioni e opportunità, con il tipo controllato dal database e non dalla schermata; gestione in Impostazioni e su `/campi-personalizzati`, valori in fondo alla scheda con lo stesso aspetto dei campi nativi. Attributi, non identità: fuori da deduplicazione e abbinamento; la fusione li trasferisce. Verificato: `test:crm-unit` 244/244, `test:crm` **139/139 in 16 sezioni eseguito il 2026-08-29** (sezione 15 compresa; controprova senza migrazione rossa esattamente lì), prova a schermo su tenant usa-e-getta rimosso — it/de/fr (U+202F misurato nel DOM), 375px senza scroll orizzontale, tema chiaro e scuro |
...
| Email CRM (0048, Fase 1.1) | migrazione **applicata in produzione il 2026-08-30**; `send-crm-email` e `crm-email-webhook` **ACTIVE** | l'Inbox resta Gmail/Microsoft readonly; l'invio umano CRM passa da Resend, non dalla Gmail API. `send-crm-email` rifiuta destinatari non registrati e dichiara `EMAIL_NOT_CONFIGURED` se manca il provider. `crm-email-webhook` verifica la firma Svix, deduplica `svix-id`, protegge dagli eventi fuori ordine e rende visibili `inviata` / `consegnata` / `fallita`; solo la consegna aggiorna l'ultimo contatto. Verificato il 2026-08-30: prova offline dedicata 12/12 senza invii veri; `test:crm` **154/154 sul database reale**, inclusa pulizia senza residui; funzioni attive con i flag JWT previsti e webhook senza secret fermato con `503 EMAIL_WEBHOOK_NOT_CONFIGURED`. Per rendere disponibile l'invio restano la configurazione di `NOTIFICATION_EMAIL_API_KEY`, `CRM_EMAIL_FROM_DOMAIN`, `RESEND_WEBHOOK_SECRET` e del webhook Resend. |
