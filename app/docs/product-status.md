...
| Clienti | `/clienti` | sì | — | sì | sì | sì | sì | Zefix (facoltativo) | l'abbinamento automatico non collega mai da solo: propone. ⚠️ **Import CSV (2026-08-27)**: la logica è coperta da `test:crm-unit` (216/216 verdi), ma la sezione 14 di `test:crm` è scritta e **non eseguita** (manca `.env.test` su questa macchina) e la prova nel browser non è stata fatta — i due passi obbligatori prima di considerarla in esercizio |
