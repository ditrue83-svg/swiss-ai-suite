...
| Clienti | `/clienti` | sì | — | sì | sì | sì | sì | Zefix (facoltativo), Resend per email | L'abbinamento automatico propone e non collega mai da solo. Import CSV e campi personalizzati sono applicati e provati. ✅ **Preventivi PDF (0049, Fase 1.2)**: migrazione applicata, `generate-crm-quote` ACTIVE v1, `send-crm-email` ACTIVE v4, Cloudflare verde e `test:crm` **176/176** sul database reale il 2026-09-01. La schermata autenticata dei preventivi in tema scuro non è ancora stata verificata online. ⛔ **Sequenze follow-up (0050, Fase 1.3)**: implementate solo nel branch `improve/crm-follow-up`, non applicate né pubblicate. Configurazione per fase e passi, silenzio misurato da `direction`, attività + notifica, template solo proposto; nessun invio automatico. Verifica locale finora: `test:crm-unit` **253/253**, `test:workflows-unit` **151/151**. La sezione reale della suite richiede la 0050. |
...
- **Deployato / configurato:** no; la migrazione 0050 non è applicata e il
  worker con la nuova scansione non è pubblicato.
- **Testato offline:** sì, `test:crm-unit` 253/253 e
  `test:workflows-unit` 151/151.
- **Testato sul database reale:** no; la sezione 18 di `test:crm` è pronta ma
  deve fallire per relazione mancante finché la 0050 non viene applicata.
- **Servizio reale / clienti esterni:** non applicabile come servizio esterno;
  ...
  funzione non è disponibile ai clienti finché migrazione e worker non sono
  pubblicati.
